...
- **api**（FastAPI + SQLAlchemy）：在 PostgreSQL 中以 `SELECT ... FOR UPDATE` 行级锁
  ...
  内先锁定路径指定的吊杆，再确认装载记录仍归属于该杆，以新旧重量差重新核算容量，只更新重量。
...
- 修正重量：把明细中一片已登记配重片的标称重量改为新的整数克数，**只更新该记录的
  `weight_grams`**，配重标识与最初登记时间保留，不删除、不重新登记；新重量沿用单片
  范围 100～25000 克，且修正后合计（当前总重 − 旧重量 + 新重量）≤ 核定值才允许，
  恰好相等允许。同一事务先锁定路径指定的吊杆再确认记录归属，越界或超载时拒绝且原重量
  保持不变；记录已被其他终端转移走时返回 `POSITION_CHANGED`，页面提示当前位置已变化
  并重新加载明细。
...
| POST | `/api/battens/{id}/loads/{load_id}/correct-weight` | 修正明细中一片的重量，体：`{"weight_grams": 12000}`；响应含修正前后重量与该杆刷新后的总重 / 余量 |
